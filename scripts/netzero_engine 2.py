...
def calculate_solar_potential(erf_number: str, suburb: str | None = None) -> dict:
    ...
    # Use roof area as proxy for GFA (single storey assumption for screening)
    estimated_gfa = roof_area
...
def netzero_scorecard(
    ...
    if nz_ratio >= 1.2:
        ...
        scores["energy"] = round(20 + (nz_ratio - 0.75) * 40)
    elif nz_ratio >= 0.5:
        scores["energy"] = round(10 + (nz_ratio - 0.5) * 40)
    else:
        scores["energy"] = round(nz_ratio * 20)
